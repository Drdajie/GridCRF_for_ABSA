...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        for j in trange(trainset.batch_count):
            ...
            preds = model(tokens, lengths, masks)
            ...
            loss = F.cross_entropy(preds, tags[:, :lengths[0], :lengths[0]], ignore_index=-1)
...
def eval(model, dataset, args):
    ...
    with torch.no_grad():
        ...
        for i in range(dataset.batch_count):
            ...
            preds_padded = torch.zeros(preds.shape[0], args.max_sequence_len, args.max_sequence_len)
            preds_padded[:, :preds.shape[1], :preds.shape[1]] = preds
            preds = preds_padded
